...
可选地，安装当前用户的 Windows 计划任务。它会在登录后启动监督器；守护器意外退出时，监督器会在约两秒内重启它。监督器本身意外退出时，计划任务会在下一分钟恢复并接管现有进程。安装完成后会立即接管当前会话，无需重新登录。守护器检测到 Codex 进程后会启动提醒宿主和工作台后台服务，Codex 关闭后再停止它们：
